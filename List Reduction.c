...
__global__ void total(float *input, float *output, int len) {
  ...
  __shared__ float array[BLOCK_SIZE*2];
  array[2*tx] = (bx*2 + tx *2) < len ? input[bx*2 + tx *2] : 0;
  array[2*tx + 1] = (bx*2 + tx *2 + 1) < len ? input[bx*2 + tx *2 + 1] : 0;
  ...
  if(tx == 0) output[blockIdx.x] = array[0];
